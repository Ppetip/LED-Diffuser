let W=28,H=10,N=W*H;
const DEFAULT_W=28,DEFAULT_H=10,MAX_FRAMES=24;
...
let hardwareProfile={width:DEFAULT_W,height:DEFAULT_H,pixels:DEFAULT_W*DEFAULT_H,source:"default"};
let project={name:"My wall show",frameMs:250,hardwareProfile:{width:DEFAULT_W,height:DEFAULT_H},frames:[blank()]};

function blank(){return Array(N).fill("#000000")}
function resizeFrame(frame,sourceWidth,sourceHeight,targetWidth,targetHeight){
  if(sourceWidth===targetWidth&&sourceHeight===targetHeight)return frame.slice(0,targetWidth*targetHeight);
  const output=Array(targetWidth*targetHeight).fill("#000000");
  for(let y=0;y<targetHeight;y++)for(let x=0;x<targetWidth;x++){
    const sourceX=Math.min(sourceWidth-1,Math.floor(x*sourceWidth/targetWidth));
    const sourceY=Math.min(sourceHeight-1,Math.floor(y*sourceHeight/targetHeight));
    output[y*targetWidth+x]=frame[sourceY*sourceWidth+sourceX]||"#000000";
  }
  return output;
}
function configureHardwareProfile(width,height,source="device"){
  width=Math.round(Number(width));height=Math.round(Number(height));
  if(!Number.isFinite(width)||!Number.isFinite(height)||width<1||height<1||width>64||height>64||width*height>2048)throw Error("Unsupported hardware profile");
  if(width===W&&height===H){
    hardwareProfile={width,height,pixels:width*height,source};
    project.hardwareProfile={width,height};
    return;
  }
  const oldWidth=W,oldHeight=H;
  project.frames=project.frames.map(frame=>{
    let sourceWidth=oldWidth,sourceHeight=oldHeight;
    if(frame.length===DEFAULT_W*DEFAULT_H){sourceWidth=DEFAULT_W;sourceHeight=DEFAULT_H}
    return resizeFrame(frame,sourceWidth,sourceHeight,width,height);
  });
  W=width;H=height;N=W*H;
  hardwareProfile={width:W,height:H,pixels:N,source};
  project.hardwareProfile={width:W,height:H};
  initGrid();draw();
  document.documentElement.style.setProperty("--matrix-width",W);
  document.documentElement.style.setProperty("--matrix-height",H);
  const badge=document.querySelector(".ambient-only");
  if(badge)badge.textContent=`${W} x ${H} ambient display`;
  appendTransportLog(`Hardware profile ${W}x${H} (${N} pixels)`);
}
...
async function probeDeviceStatus(){
  ...
  if(statusReply?.width&&statusReply?.height)configureHardwareProfile(statusReply.width,statusReply.height,"device");
...
function initGrid(){
  const matrix=$("matrix");
  matrix.innerHTML="";
  matrix.style.gridTemplateColumns=`repeat(${W},minmax(0,1fr))`;
  matrix.style.aspectRatio=`${W} / ${H}`;
  for(let i=0;i<N;i++){
...
function loadProject(next){
  ...
  const declared=next.hardwareProfile||{width:next.width||DEFAULT_W,height:next.height||DEFAULT_H};
  const sourceWidth=+declared.width||DEFAULT_W,sourceHeight=+declared.height||DEFAULT_H;
  next.frames=next.frames.slice(0,MAX_FRAMES).map(frame=>{
    let frameWidth=sourceWidth,frameHeight=sourceHeight;
    if(frame.length===DEFAULT_W*DEFAULT_H){frameWidth=DEFAULT_W;frameHeight=DEFAULT_H}
    if(frame.length!==frameWidth*frameHeight)throw Error(`Frame has ${frame.length} colors; expected ${frameWidth*frameHeight}`);
    return resizeFrame(frame,frameWidth,frameHeight,W,H);
  });
  next.hardwareProfile={width:W,height:H};
...
$("exportSchema").onclick=()=>downloadJson("led-diffuser-schema-v2.json",{schemaVersion:2,width:W,height:H,hardwareProfile,blendModes:LEDCompiler.BLEND_MODES,blocks:LEDCompiler.schema()});
...
function insertBlockExample(type){
  ...
  if(!program)program={schemaVersion:2,name:"Custom LED Program",width:W,height:H,frameMs:100,frameCount:12,brightness:1,layers:[]};
